...
Function single
{
    ...
    else
    {
        ...
        {
            New-item -Path "$($SaveDir)\txt\$($BadgeNumber)\badge.txt" -ItemType Directory | Out-null -ErrorAction SilentlyContinue
        }
    ...
    Copy-Item -Path "C:\Users\legon\OneDrive\Documents\Powershell\Badge2040\badge.py" -Destination "$($SaveDir)\txt\$($Entry.BadgeNumber)\badge.py"
    Copy-Item -Path "C:\Users\legon\OneDrive\Documents\Powershell\Badge2040\badge-image.bin" -Destination "$($SaveDir)\txt\$($Entry.BadgeNumber)\badge-image.bin"
    $Rshell = rshell --version
    if ($Rshell -match "0.0.3*")
    {
        Set-Location -Path "$SaveDir\txt\"
